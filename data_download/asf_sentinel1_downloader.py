"""Download de imagens Sentinel-1 diretamente do portal ASF Vertex.

Edite os parametros na secao CONFIGURACAO e execute o arquivo diretamente.

O fluxo usa a biblioteca oficial asf_search para localizar cenas Sentinel-1
por janela temporal, AOI, plataforma, orbita relativa e direcao de voo.
...
DEFAULT_OUTPUT_DIR = ROOT_DIR / "downloads" / "sentinel1_asf"
...
DATE_END   = "2026-04-30T23:59:59Z"   # fim da busca  (None = agora)
PROCESSING_LEVEL = asf.PRODUCT_TYPE.SLC   # "SLC", "GRD_HD", "GRD_MS", etc.
WINDOW_MONTHS = 1
PLATFORM_FILTERS: list[str] = [asf.PLATFORM.SENTINEL1A, asf.PLATFORM.SENTINEL1C]
RELATIVE_ORBIT_FILTERS: list[int] | None = [24,39,53,68,97,112,126,141,155,170]   # Ex: [24, 131]
...
# Ou adicione ao credentials.txt (na raiz do projeto):
...
def search_window(
	...
	kwargs: dict[str, Any] = {
		"platform": PLATFORM_FILTERS or [asf.PLATFORM.SENTINEL1],
		"processingLevel": [PROCESSING_LEVEL],
		"start": start,
		"end": end,
	}
	if search_wkt:
		kwargs["intersectsWith"] = search_wkt
	if relative_orbit is not None:
		kwargs["relativeOrbit"] = relative_orbit
	if FLIGHT_DIRECTION:
		kwargs["flightDirection"] = FLIGHT_DIRECTION
	if MAX_RESULTS_PER_QUERY is not None:
		kwargs["maxResults"] = MAX_RESULTS_PER_QUERY
...
def main() -> None:
	...
	if not auth_available:
		if has_esa_maap_credentials_only():
			print(
				"[SENTINEL-1] credentials.txt contem credenciais ESA MAAP "
			...
			print(
				"[SENTINEL-1] Nenhuma credencial Earthdata encontrada. "
	...
	# Collect all matching products, deduplicated by fileID
	seen: dict[str, asf.ASFProduct] = {}
	total_found = 0

	product_list = read_product_list(PRODUCT_LIST_TXT)
	if product_list:
		print(f"[SENTINEL-1] Modo lista: buscando {len(product_list)} produto(s) em {PRODUCT_LIST_TXT}", flush=True)
		try:
			results = search_by_product_list(product_list)
		except Exception as err:
			print(f"[SENTINEL-1] Busca por lista falhou: {err}", flush=True)
			results = []
		total_found = len(results)
		for product in results:
			pid = product.properties.get("fileID") or product.properties.get("sceneName", "")
			if pid:
				seen[pid] = product
	else:
		...
		for start, end in windows:
			for relative_orbit in relative_orbits:
				try:
					...
					print(
						f"[SENTINEL-1] Busca falhou ({start} -> {end}, "
				...
				total_found += len(results)
				print(
					f"[SENTINEL-1] {len(results)} cena(s) | {start} -> {end} | "
				...
				for product in results:
					...
					if pid and pid not in seen:
						if product_intersects_validation(product, validation_geometry):
							seen[pid] = product

	print(f"[SENTINEL-1] Total bruto da busca: {total_found}", flush=True)
	...
	for idx, product in enumerate(products_list, start=1):
		...
		manifest_rows.append({
			...
			"absolute_orbit": props.get("absoluteOrbit", ""),
